...
def is_module_enabled(guild_id, module: str) -> bool:
    """Prueft ob ein Modul auf diesem Server aktiviert ist"""
    return load_data("config.json").get(str(guild_id), {}).get(f"module_{module}", False)

def get_log_channel_id(guild_id) -> str | None:
    """Gibt die Log-Channel ID zurueck oder None"""
